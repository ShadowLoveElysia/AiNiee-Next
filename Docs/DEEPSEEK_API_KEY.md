...
这里用 DeepSeek 做示范，是因为它整体比较便宜、翻译效果够用、速度和稳定性也适合新手先跑通流程。它不是唯一能用的平台，只是很适合作为第一次配置 API 的例子。

> API Key 相当于你的付费调用密码。创建后一定要保存好，不要发给别人，不要截图公开，不要贴到群里或 Issue 里。别人拿到你的 Key 后可以消耗你的账户余额，也就是会直接扣你的钱。

## 1. 登录 DeepSeek 开放平台

打开 DeepSeek 开放平台后，按页面显示的方式登录或注册。国内站通常可以使用手机号验证码或微信扫码登录；海外站可能以邮箱注册/登录为主。具体入口以 DeepSeek 当前页面显示为准。

<p align="center">
  <img src="../README_IMG/DeepSeekAPI/1.png" alt="图 1：DeepSeek 开放平台登录页" width="78%">
  <br>
  <sub>图 1：进入 DeepSeek 开放平台，按页面提示完成登录或注册。</sub>
</p>
