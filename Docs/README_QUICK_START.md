...
1. 语言选择：`1. 中文（简体）`
2. 操作方式：本教程使用 `2. 传统模式（纯 API）`
3. 源语言：保持 `auto`
4. 目标语言：保持 `Chinese`
5. API 配置：选择 `1. 在线 API（预设）`
6. 在线 API 预设：选择 `5. deepseek`

如果选择 `1. 使用智能助手（Agent）`，向导会直接显示外部 Agent 接入指引，跳过翻译语言、API 选择和 API 验证。后续由智能助手根据用户文件确定源语言并确认目标语言。
