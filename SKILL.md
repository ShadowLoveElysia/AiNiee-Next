...
- MCP 连接成功后依次读取 `get_mcp_usage_manual(section="overview")`、`get_mcp_security_policy`、`get_mcp_tool_categories`、目标 `get_mcp_tool_catalog`，再注册会话、请求任务模式并准备项目。默认速度优先，用 `agent_claim_batches`（读取当前 Profile 配置，默认 8，可由用户设为更大值）给多个 SubAgent 分配工作；`agent_claim_batch(batch_id=...)` 支持指定批次。正式翻译调用 `agent_submit_translation_batch` 即完成校验和缓存写回，默认 `auto_commit=true`，服务端管理 writer lease 和串行写入；收到 `committed` 后不要再逐批领取租约/重复写回。只有明确需要暂存时使用 `auto_commit=false`，之后可调用 `agent_commit_cache_batch`，省略 writer_lease_id 由服务端管理。普通逐行 TXT 的直接项目没有缓存时仍只暂存，不宣称导出完成。
...
- 结束前调用 `agent_pending_work` 检查所有未完成项；只有 `all_committed=true` 才可导出并确认完成。全部 committed 后 MCP 尝试自动导出；缺少运行时上下文或导出失败时按回执调用 `agent_export_task`，没有导出成功结果不得声称电子书已生成。断线后新会话通过 `agent_resume_task` 接管，已拒绝候选、待写回结果和恢复凭据保留。API key、MCP token 和用户原文不写入日志或配置。
