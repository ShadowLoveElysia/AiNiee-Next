...
`recover_task` 用于新会话在传输断线后接管已有任务：新会话必须先注册并通过 `request_external_mode`，服务会校验持久化 source/cache 指纹并复用已预热缓存，不重新解析 EPUB；可选 expected_input_hash/expected_cache_revision 用于额外校验。`resume_task` 仍用于需要明确旧会话审计接管的场景；恢复后提交或重试 commit 时由服务端重新获取 writer lease。旧会话不能继续提交或写回。

`agent_session` 还提供 `request_external_mode`，通过已鉴权的 Skills 端口给当前 session 请求运行时 `external_agent` 模式。它不会写入 Profile、`Resource/config.json` 或 `translation_execution_mode`；请求前必须完成 onboarding、注册 session 并在注册时提供用户确认。随后才可调用受控批次动作：`prepare_project`、`prepare_cache_project`、`project_status`、`claim_batch`、`claim_batches`、`submit_translation_batch`、`recover_task`、`pending_work`、`get_batch_repair`、`release_batch`、`resume_task`、`acquire_writer_lease` 和 `commit_cache_batch`。默认速度优先时使用 `claim_batches` 进行有界并行领取，也可给 `claim_batch` 传 `batch_id` 跳批次；质量优先时仍可逐批串行处理。批次动作必须携带有效的 Agent session；输入路径受 Skills 工作区边界限制。`submit_translation_batch` 默认自动校验并写回缓存（`auto_commit=true`），由服务端管理 writer lease；无缓存 TXT 项目仍只暂存。`auto_commit=false` 可保留暂存模式；`commit_cache_batch` 用于重试或人工提交，writer_lease_id 可省略。Skills 不接受 API key、MCP token 或任意内部路径，也不会让 Agent 直接改写缓存或输出文件。

`submit_translation_batch` 的 `items` 使用 `{index, translation}`；正常回执不重复返回原文/译文。`repair_required` 时使用 `get_batch_repair` 查看问题，再以 `repair=true` 和新幂等键提交修正条目；三次不同失败候选后 `needs_review`，停止自动循环并报告。`submitted` 加 `write_error` 表示需要重试写回或处理冲突，不要盲目重译。最后查询 `pending_work`，只有全部 committed 才完成缓存写回；本备用服务本身不执行 MCP 的自动电子书导出，不能仅凭缓存完成声称已生成输出。
