...
export const ElysiaTheme: React.FC<{ active: boolean }> = ({ active }) => {
  ...
  useEffect(() => {
    if (active) {
      document.documentElement.classList.add('elysia-mode');
      document.body.classList.add('elysia-transforming');
      setTimeout(() => document.body.classList.remove('elysia-transforming'), 1000);
      
      console.log("%c嗨~ 发现我了？这是我们的小秘密哦~ ✨", "color: #ff6699; font-size: 20px; font-weight: bold; text-shadow: 2px 2px 4px rgba(255,102,153,0.3);");
      console.log("%c让我们一起把这些文字，变成像水晶一样闪耀的东西吧~", "color: #d4a5ff; font-size: 14px;");

      // Generate background crystal shards
      const newParticles = Array.from({ length: 20 }).map((_, i) => ({
        ...
        shape: Math.random() > 0.5 ? 'polygon(50% 0%, 100% 50%, 50% 100%, 0% 50%)' : 'polygon(25% 0%, 100% 0%, 75% 100%, 0% 100%)'
      }));
  ...
  return (
    <>
      {active && (
        <div className="elysia-overlay-container">
          {/* Global Atmosphere Glow */}
          <div className="elysia-glow-orb elysia-glow-1"></div>
          <div className="elysia-glow-orb elysia-glow-2"></div>

          {/* Background Crystal Shards */}
          {particles.map(p => (
            <div 
              key={p.id}
              className="elysia-bg-shard"
              style={{ 
                left: `${p.x}%`, 
                top: `${p.y}%`, 
                width: `${p.size}px`, 
                height: `${p.size}px`,
                clipPath: (p as any).shape,
                transform: `rotate(${(p as any).rotate}deg)`,
          ...
          {trail.map((t, i) => (
            <div 
              key={t.id}
              className="elysia-mouse-trail"
              style={{ 
                left: t.x, 
                top: t.y, 
                opacity: (24 - i) / 60,
                transform: `translate(-50%, -50%) scale(${(24 - i) / 24})`
              }}
            />
          ))}
          ...
          {clicks.map(click => (
            <div 
              key={click.id}
              className="elysia-click-sparkle"
